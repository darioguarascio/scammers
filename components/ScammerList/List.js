...
const List = ({ data, onSetPage, onSetPerPage, total, pageInfo, addSort }) => {
    ...
    <div>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <PerPage onSetPerPage={(perPage) => onSetPerPage(perPage)} />
        </div>
      </div>
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg my-4">
        <table className="w-full text-sm text-left text-gray-500">
            ...
            <tr>
              <th scope="col" className="px-6 py-1 cursor-pointer">
                <div className="flex flex-row items-center gap-x-2">
                  <span onClick={() => changeSort("codeName")}>
                    Codenme of scammer
                  </span>
                  <div className="grid">
                    <div
          ...
          <tbody>
            {items.length > 0 &&
              items.map((item) => <Item item={item} key={item.codename} />)}
          </tbody>
        </table>
      </div>
      {items.length > 0 && (
